...
function SectionCard({ title, subtitle, children }) {
  return (
    <Card
      sx={{ borderRadius: 4, boxShadow: "0 24px 60px rgba(15, 23, 42, 0.08)" }}
    >
      <CardContent sx={{ p: 3 }}>
...
function AdminDashboard() {
  ...
  if (loading)
    return <Alert severity="info">Cargando panel administrativo...</Alert>;
  ...
  return (
    <Stack spacing={3}>
      ...
      <SectionCard
        ...
        subtitle="Desde aqui puedes verificar rapidamente el rol y los datos principales."
      >
        <Table size="small">
          <TableHead>
            <TableRow>
...
function PatientDashboard({ usuario }) {
  ...
  useEffect(() => {
    cargar().catch(() =>
      Swal.fire(
        "Error",
        "No fue posible cargar el panel del paciente.",
        "error",
      ),
    );
  }, [usuario.id]);
  ...
  const handleCrearCita = async (event) => {
    ...
    try {
      ...
      Swal.fire(
        "Cita creada",
        "La cita fue registrada y quedo lista para notificacion por WhatsApp.",
        "success",
      );
      setFormData({
        psicologoId: "",
  ...
  if (!resumen)
    return (
      <Alert severity="info">Cargando tus citas y tu historia clinica...</Alert>
    );
  ...
  return (
    <Stack spacing={3}>
      <SectionCard
        ...
        subtitle="Aqui puedes consultar tus citas programadas y pedir nuevas sesiones."
      >
        <Stack spacing={2}>
          {citas.length === 0 ? (
            <Alert severity="warning">Aun no tienes citas registradas.</Alert>
          ) : null}
          {citas.map((cita) => (
            <Paper
              key={cita.id}
            ...
            >
              ...
              >
                <Box>
                  ...
                  <Typography color="text.secondary">
                    Psicologo: {cita.psicologo?.nombre}{" "}
                    {cita.psicologo?.apellido}
                  </Typography>
                </Box>
                <Chip
                  label={cita.estado}
                  color={estadoColor[cita.estado] || "default"}
                />
              </Stack>
            </Paper>
          ))}
      ...
      <SectionCard
        ...
        subtitle="Cada cita queda enlazada con tu telefono para aviso de cambios por WhatsApp."
      >
            ...
            <FormControl fullWidth required>
              ...
              <Select
                ...
                onChange={(event) =>
                  setFormData((current) => ({
                    ...current,
                    psicologoId: event.target.value,
                  }))
                }
              >
                {psicologos.map((item) => (
      ...
      <SectionCard
        ...
        subtitle="El paciente puede revisar un resumen general de su proceso."
      >
        {historia ? (
          <Stack spacing={1.5}>
            <Typography>
            ...
            <Typography>
              <strong>Antecedentes:</strong> {historia.antecedentes}
            </Typography>
            <Typography>
              <strong>Diagnostico inicial:</strong>{" "}
              {historia.diagnosticoInicial}
            </Typography>
            <Typography>
              <strong>Plan de tratamiento:</strong> {historia.planTratamiento}
            </Typography>
          </Stack>
        ) : (
          <Alert severity="info">
            Tu historia clinica aun no ha sido diligenciada por el terapeuta.
          </Alert>
        )}
      </SectionCard>
    </Stack>
...
function PsychologistDashboard({ usuario }) {
  ...
  useEffect(() => {
    cargarBase().catch(() =>
      Swal.fire("Error", "No se pudo cargar el panel del psicologo.", "error"),
    );
  }, [usuario.id]);
  ...
  const guardarHistoria = async (event) => {
    ...
    try {
      ...
      Swal.fire(
        "Historia guardada",
        "La historia clinica fue actualizada.",
        "success",
      );
  ...
  const guardarEvolucion = async (event) => {
    ...
    try {
      ...
      Swal.fire(
        "Registro agregado",
        "La evolucion clinica fue almacenada.",
        "success",
      );
      setEvolucionForm({
        citaId: "",
  ...
  const cambiarEstado = async (citaId, estado) => {
    try {
      ...
      Swal.fire(
        "Cita actualizada",
        `La cita quedo ${estado.toLowerCase()}.`,
        "success",
      );
      await cargarBase();
  ...
  return (
    <Stack spacing={3}>
      <SectionCard
        title="Agenda del psicologo"
        subtitle="Controla citas programadas, reprogramadas, completadas o canceladas."
      >
        <Stack spacing={2}>
          {agenda.map((cita) => (
            <Paper
      ...
      <SectionCard
        ...
        subtitle="Selecciona un paciente para consultar su historial completo y registrar evoluciones."
      >
        <Stack spacing={2}>
          ...
          {pacienteSeleccionado ? (
            <Stack spacing={3}>
                ...
                <Stack spacing={2}>
                  ...
                  <TextField
                    ...
                    onChange={(event) =>
                      setHistoriaForm((current) => ({
                        ...current,
                        antecedentes: event.target.value,
                      }))
                    }
                  />
                  <TextField
                    ...
                    multiline
                    minRows={2}
                    value={historiaForm.diagnosticoInicial}
                    onChange={(event) =>
                      setHistoriaForm((current) => ({
                        ...current,
                        diagnosticoInicial: event.target.value,
                      }))
                    }
                  />
                  <TextField
                    ...
                    multiline
                    minRows={2}
                    value={historiaForm.planTratamiento}
                    onChange={(event) =>
                      setHistoriaForm((current) => ({
                        ...current,
                        planTratamiento: event.target.value,
                      }))
                    }
                  />
                  <Button
              ...
              <Box component="form" onSubmit={guardarEvolucion}>
                ...
                <Stack spacing={2}>
                  <FormControl fullWidth>
                    ...
                    <Select
                      label="Cita asociada"
                      value={evolucionForm.citaId}
                      onChange={(event) =>
                        setEvolucionForm((current) => ({
                          ...current,
                          citaId: event.target.value,
                        }))
                      }
                    >
                      ...
                      {citasPaciente.map((cita) => (
                        <MenuItem key={cita.id} value={cita.id}>
                          {formatearFecha(cita.fechaHora)} - {cita.estado}
                        </MenuItem>
                      ))}
                    </Select>
                  </FormControl>
                  <TextField
                    ...
                    minRows={2}
                    required
                    value={evolucionForm.resumenSesion}
                    onChange={(event) =>
                      setEvolucionForm((current) => ({
                        ...current,
                        resumenSesion: event.target.value,
                      }))
                    }
                  />
                  <TextField
                    ...
                    multiline
                    minRows={2}
                    value={evolucionForm.observaciones}
                    onChange={(event) =>
                      setEvolucionForm((current) => ({
                        ...current,
                        observaciones: event.target.value,
              ...
              <Box>
                ...
                {!detalle?.evoluciones?.length ? (
                  <Alert severity="info">
                    Aun no hay evoluciones registradas.
                  </Alert>
                ) : null}
                <Stack spacing={1.5}>
                  {detalle?.evoluciones?.map((item) => (
                    ...
                    >
                      ...
                      <Typography color="text.secondary">
                        Recomendaciones:{" "}
                        {item.recomendaciones || "Sin recomendaciones"}
                      </Typography>
                    </Paper>
                  ))}
                </Stack>
            ...
            <Alert severity="info">
              Selecciona un paciente para abrir la historia clinica.
            </Alert>
          )}
        </Stack>
      </SectionCard>
...
export function DashboardPage({ usuario, onLogout }) {
  ...
  const subtitulo = {
    ADMINISTRADOR:
      "Control general de usuarios, agendas y trazabilidad del sistema.",
    PSICOLOGO:
      "Gestion de pacientes, historia clinica, agenda y seguimiento terapeutico.",
    PACIENTE:
      "Consulta tus citas y solicita nuevas sesiones desde tu propio ambiente.",
  }[usuario.rol];
    ...
    >
      <Container maxWidth="lg" sx={{ py: 5 }}>
        ...
        >
          ...
          >
            <Box>
              ...
              <Typography sx={{ opacity: 0.85, maxWidth: 720 }}>
                {subtitulo}
              </Typography>
            </Box>
        ...
        {usuario.rol === "PACIENTE" ? (
          <PatientDashboard usuario={usuario} />
        ) : null}
        {usuario.rol === "PSICOLOGO" ? (
          <PsychologistDashboard usuario={usuario} />
        ) : null}
      </Container>
    </Box>
  );
